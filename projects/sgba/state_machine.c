...
// another crazyflie with an lower ID is coming close, 3=SGBA: The SGBA method
// that incorperates the above methods.
//        NOTE: the switching between outbound and inbound has not been
//        implemented yet
#define METHOD 1
...
void appMain(void *param) {
	...
	while (1) {
		...
		for (uint8_t it = 0; it < 9; it++)
			if (usecTimestamp() >= time_array_other_drones[it] + 1000 * 1000) {
		...
		rssi_inter_filtered = (uint8_t)movingAvg(arrNumbers_inter, &sum_inter,
		pos_avg_inter, len_inter, (int)rssi_inter_closest);*/
		...
		uint8_t multiranger_isinit = paramGetVarIddeckbcMultiranger();
		uint8_t flowdeck_isinit = paramGetVarIddeckbcFlow2();

		// get current height and heading
		height = logGetVarIdkalmanstateZ();
		float heading_deg = logGetVarIdstabilizeryaw();
		heading_rad = heading_deg * (float)M_PI / 180.0f;

		// t RSSI of beacon
		rssi_beacon = logGetVarIdradiorssi();
		...
		if (multiranger_isinit) {
			front_range = (float)rangeGetrangeFront() / 1000.0f;
			right_range = (float)rangeGetrangeRight() / 1000.0f;
			left_range = (float)rangeGetrangeLeft() / 1000.0f;
			back_range = (float)rangeGetrangeBack() / 1000.0f;
			up_range = (float)rangeGetrangeUp() / 1000.0f;
		}
